...
You need a Goober for each Sender Unit and Receiver Unit. You have two choice for the Goober.

- You can have the Goober PCB manufactured and shipped to you by a PCB manufacturer.

- You can either build a Goober from some basic parts as explained in below.
...
I want to order the Goober PCB to be manufactured
-------------------------------------------------

The Goober PCB is an open-hardware board designed by me for this project. The KiCad files and the BOM are available from `my GitHub repo <https://github.com/aminjahanpour/elora_vision_pcb>`_.

The Goober PCB is a compact (4cm by 4cm) 4-layer board powered via its USB mini connection.
...
|
|
I want to build my own Goober
-----------------------------

Here are the ingredients needed for one Goober.
...
3) 8 jumper wires
    These wires are used to connect LoRa module to the board. Choose shorter cables to keep the Goober small and tidy.
...
You have a Goober now. It should look like this one below.


.. image:: ./images/black_pill_goober.jpg
    :align: center
...
How to flash the Goober Firmware
--------------------------------

Regardless of how you build a Goober, you need to flash the firmware to it.
...
But you do need to download the firmware and flash it to your Goober once.

Here is how you'd go about uploading the firmware to the Goober.


1) `Get STM32CubeProgrammer software from here <https://www.st.com/en/development-tools/stm32cubeprog.html>`_ and install it on your computer. You will use this application to flash the firmware into your Goober.
...
Depending on your board, you are going to need either a regular USB cable or an `ST-Link <https://www.amazon.ca/dp/B07B2K6ZPK?psc=1&ref=ppx_yo2ov_dt_b_product_details>`_ programmer to connect the Goober to your computer.

If you are using a USB cable here are the steps you need to take to flash the firmware to your boards.


3) Connect the Goober to your computer via a USB cable.
...
9) Finally click on `Download` button. After few seconds all is done and you have the firmware installed on the Goober.
...
The Goober PCB however, can only be programmed using the ST-Link (not by a USB cable). The board has the SWD pins which make it easy for you to flash the firmware into the chip.
...
You can simply plug the linker, choose ST-Link on the program, press connect and download the firmware to Goober.


.. note::
    Same exact firmware is used on all Goobers (sender or receiver).
